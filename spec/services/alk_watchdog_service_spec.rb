...
RSpec.describe AlkWatchdogService do
  ...
  describe '.call' do
    ...
    it 'persists a flat Trend when alk readings have no variance, rather than skipping it' do
      alk_reading value: 8.0, recorded_at: 2.hours.ago
      alk_reading value: 8.0, recorded_at: 1.hour.ago

      described_class.call

      expect(Trend.last).to have_attributes(slope: 0.0, r_squared: 1.0)
    end

    it 'takes no action when alk readings have no variance, since a flat trend is never rising' do
      alk_reading value: 8.0, recorded_at: 2.hours.ago
      alk_reading value: 8.0, recorded_at: 1.hour.ago
